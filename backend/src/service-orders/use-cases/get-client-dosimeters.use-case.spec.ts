...
describe('GetClientDosimetersUseCase', () => {
  ...
  it('filters out dosimeters whose physical status is not ASIGNADO', async () => {
    orgScope.getOrganizationType.mockResolvedValue('laboratory');
    ...
    const fakeRows = [
      {
        ...
        dosimeters: {
          id: 'dos-1',
          serial_number: 'SN-001',
          dosimeter_statuses: { id: 'st-1', code: 'EN_TRANSITO', name: 'En tránsito' },
        },
        workers: { id: 'w-1', client_id: 'client-1', full_name: 'Juan Perez' },
      },
      {
        id: 'assign-2',
        dosimeter_id: 'dos-2',
        ...
        dosimeters: {
          ...
          dosimeter_statuses: { id: 'st-2', code: 'ASIGNADO', name: 'Asignado' },
        },
        workers: { id: 'w-2', client_id: 'client-1', full_name: 'Maria Gomez' },
      },
    ];
    ...
    const result = await useCase.execute('client-1', 'org-lab');
    expect(result).toHaveLength(1);
    expect(result[0].dosimeter_id).toBe('dos-2');
  });
